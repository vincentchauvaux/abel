...
- **Gardien** : même API avec `role: guardian` (jusqu’à 5) ; `DELETE /members/:userId` pour révoquer. Le gardien note les entrées, ne peut pas modifier identité / objectifs / liste d’exercices (`babies`, `reminderRules` ignorés au `POST /sync` ; `exerciseItems` : seulement `startedAt`).
...
- **Pas de modification** de la fiche Bébé (identité, photo, date de naissance, objectifs / rappels, liste d’exercices). Lecture seule sur Identité, Objectifs et Exercices ; le réglage rappel du module Allaitement est masqué. Peut **démarrer / terminer** les comptes à rebours du dashboard.
