...
Cartes **Favoris** (si configurés, sinon **+** / select pour en ajouter) : raccourcis vers les outils épinglés depuis chaque module (cœur à droite du titre). Puis **Graphiques** (juste sous Favoris) : sur **Jour**, une **timeline 24 h** unique tétées (terracotta) et siestes (bleu), positionnées par début/fin (chevauchement possible, ligne « maintenant ») ; une séance dont le **début est dans le futur** n’est pas affichée ; camembert couches (pipi / caca / les deux). Sur **7 j / 30 j / Tout** : totaux par jour (repas, sommeil, couches), **dernier jour à droite** et défilement vers la gauche pour le passé ; sur **Tout**, repas / sommeil / couches : **6 derniers jours visibles**, scroll vers la gauche pour l’historique (bâtons assez larges pour lire les minutes de tétées) ; **cadran 24 h des siestes** (courbe, transparent s’il n’y a pas de sieste, plus foncé / plus large = siestes plus longues) ; sur **7 j**, interrupteur Cadran / **Agenda** (jours en lignes, heures 0–24, bâtonnets de sieste). **Repas** (7 j / 30 j / Tout) : un bâton empilé tétées / biberons (hauteur = nombre, total au-dessus) ; minutes dans le segment tétées, ml dans le segment biberons s’il y en a. **Couches** (7 j / 30 j / Tout) : bâton empilé pipi / caca / les deux (mêmes couleurs que le camembert du jour). **Poids et taille** : dates sur l’axe X, **6 dernières mesures** visibles (scroll vers la gauche pour l’historique), ligne **en pointillés** s’il manque une mesure (ex. taille). IMC indicatif vert / orange / rouge — pas un avis médical. **Accordéon Notes** puis **Alertes** repas / sommeil / couche. Notes filtrées par période (todos ouvertes toujours visibles ; todos faites selon `doneAt`). Puis **Apports** : biberon, diversification, compléments (une ligne par indicateur avec bouton **+**). Puis **Suivi** : couches, sommeil, tire-lait (stock et tiré sur la période), poids/taille/PC, temp. (même format liste avec **+**). **Entrées de la période** : journal synthétique en bas de page (avatar Google à droite si l’auteur est connu) ; **tap pour modifier** (même éditeur que le journal Bébé).
