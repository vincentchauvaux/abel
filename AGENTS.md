...
Application web pour le suivi quotidien d’un nourrisson : tétées, biberons, diversification, compléments, couches, bains, tire-lait, croissance, sommeil, température, notes, exercices. Pensée pour une maman qui a le bébé dans les bras : **1 action = 1 pression = donnée enregistrée immédiatement**.
...
Cartes **Favoris** (si configurés, sinon **+** / select pour en ajouter) : raccourcis vers les outils épinglés depuis chaque module (cœur à droite du titre), y compris les **exercices** (triangle **Démarrer** à la place du + ; décompte dans la ligne ; sonnerie à la fin). Les exercices en cours sont aussi dans le bandeau (Terminer). Sections **repliables** (Favoris, Graphiques, Poids et taille, Notes, Alertes, Apports, Suivi, Exercices) : tap sur le titre pour ouvrir/fermer ; **poignée de déplacement** seulement une fois refermées (ordre mémorisé en local `abel.dash-sections`). Ordre par défaut : Favoris → Graphiques → Poids et taille → Notes (fermée) → Alertes → Apports → Suivi → Exercices. **Entrées de la période** restent collées en bas. **Graphiques** : carte **Résumé** avec **cadran 24 h en premier** (au centre : **durée de sommeil**, total du jour ou moyenne / jour ; **trait de l’heure actuelle** sur tous les cadrans ; sur **Jour**, seulement les heures déjà passées ; une sieste commencée la veille n’apparaît que depuis 0 h) ; légende cliquable **Siestes** / **Repas** (siestes par défaut, mémorisé en local `abel.dash-clock-legend`) : on active ou désactive chaque anneau, les deux peuvent **se superposer** ; totaux et pics dans **Infos** (repliable, bas droite) ; interrupteur Cadran / **Agenda** seulement sur **Jour** et **7 j** (mémorisé en local `abel.dash-clock-view` ; aujourd’hui en haut, piste repas + piste siestes) — **pas d’agenda** sur **30 j** et **Tout**. Puis totaux par jour (**dernier jour à droite**) : sur **Jour**, un bâton par tétée et par sieste (durée en minutes, heure sous le bâton) et **camembert Couches** (pipi / caca / les deux) ; **Sommeil** en bâtons sur toutes les périodes ; **Repas** en bâtons empilés sur **7 j / 30 j / Tout** ; sur **Tout**, 3 jours visibles, bâtons larges ; **Couches** : bâton empilé pipi / caca / les deux. Sommeil par jour = minutes réellement couvertes ce jour-là (sieste à cheval coupée à minuit). **Repas** (7 j / 30 j / Tout) : bâton empilé tétées / biberons (hauteur = nombre) ; minutes dans le segment tétées, ml dans le segment biberons. Une séance dont le **début est dans le futur** n’est pas affichée ; **durées du jour** = mêmes minutes, portion après minuit y compris une sieste commencée la veille. **Couches** (7 j / 30 j / Tout) : bâton empilé pipi / caca / les deux (mêmes couleurs que le camembert du jour). Sommeil par jour = minutes réellement couvertes ce jour-là (sieste à cheval coupée à minuit). **Repas** (7 j / Tout) : un bâton empilé tétées / biberons (hauteur = nombre, total au-dessus) ; minutes dans le segment tétées, ml dans le segment biberons s’il y en a. **Poids et taille** : dates sur l’axe X, **6 dernières mesures** visibles (scroll vers la gauche pour l’historique), ligne **en pointillés** s’il manque une mesure (ex. taille). IMC indicatif vert / orange / rouge — pas un avis médical. **Notes** (fermée par défaut) puis **Alertes** repas / sommeil / couche. Notes filtrées par période (todos ouvertes toujours visibles ; todos faites selon `doneAt`). Puis **Apports** : biberon, diversification, compléments (une ligne par indicateur avec bouton **+**). Puis **Suivi** : couches, bains, sommeil, tire-lait (stock et tiré sur la période), poids/taille/PC, temp. (même format liste avec **+**). Puis **Exercices** : une ligne par intitulé (compte sur la période, dernière heure, triangle Démarrer) ; une séance terminée (ou Relancer) est **notée dans le journal** (`exercise_sessions`). **Entrées de la période** : journal synthétique en bas de page (avatar Google à droite si l’auteur est connu) ; **tap pour modifier** (même éditeur que le journal Bébé).
...
- **Gardien** : même API avec `role: guardian` (jusqu’à 5) ; `DELETE /members/:userId` pour révoquer. Le gardien note les entrées, ne peut pas modifier identité / objectifs / liste d’exercices (`babies`, `reminderRules` ignorés au `POST /sync` ; `exerciseItems` : seulement `startedAt` ; `exerciseSessions` : saisie complète comme les autres entrées).
...
- **Politique de confidentialité** — RGPD, finalités, droits, export/suppression (album chiffré sur le VPS). URL Play : `https://mimom.be/privacy.html`.
...
- **Pas de modification** de la fiche Bébé (identité, photo, date de naissance, objectifs / rappels, liste d’exercices). Lecture seule sur Identité, Objectifs et Exercices ; le réglage rappel du module Allaitement est masqué. Peut **démarrer / terminer** les comptes à rebours du dashboard (une séance est notée dans le journal).
...
Enveloppe Android autour du site, **pas** une réécriture native. Guide : [`store/android/README.md`](store/android/README.md). Dossier Console (textes, Data safety, visuels) : [`store/android/play/README.md`](store/android/play/README.md).

- Package : **`be.mimom.twa`** (PWABuilder, septembre 2026) — défini par le `.aab`, pas par « Créer une application »
- AAB / APK : zip Play (copie locale gitignorée `store/android/Mimom.aab`)
- Fiche Play : `store/android/play/` (icône 512, bannière 1024×500, captures, `fiche.txt`)
- Confidentialité Play : `https://mimom.be/privacy.html` (`public/privacy.html`)
...
- Compte Play personnel : test **fermé** 12 testeurs × 14 jours avant la production
...
babies (name, bornOn, photoUrl)
 ...
 ├── exercise_items (title, durationMinutes, startedAt)
 ├── exercise_sessions (exerciseItemId, title, durationMinutes, startedAt, endedAt)
...
Page **Bébé** : sections en **accordéon** (un seul panneau ouvert à la fois), identité et objectifs en lecture une fois renseignés (bouton Modifier ; un **gardien** n’a pas Modifier). **Exercices** : ajouter un intitulé + une durée (chips 5–60 min ou personnalisé) ; le gardien voit la liste sans Ajouter / Retirer, et peut lancer les décomptes depuis Outils. Journal chronologique éditable filtré par **jour** (aujourd’hui par défaut ; champ vide ou **Tout** = toutes les entrées, date affichée sur chaque ligne) et par type d’entrée (**Tout** coche ou décoche tous les types ; tri sur l’heure de fin pour tétées, sommeil, exercices et tire-lait chronométré), durées en **22 min** / **1 h 30 min** (comme le sommeil, pas `22:10`), **avatar** du compte à droite si l’auteur est connu. Édition journal : tétée (sein Gauche/Droit, **Les deux** si les deux seins ont servi pendant la séance, état, **date + heures de début et de fin** : changer l’une recalcule la durée ; saisir la durée décale la fin ; sieste qui dépasse minuit = fin le lendemain), sommeil et **exercice** (même logique début/fin/durée), biberon/tire-lait (ml, début/fin/durée tire-lait), couche, bain, diversification, complément, température, mesures, notes.
