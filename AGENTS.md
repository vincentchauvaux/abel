...
Cartes **Favoris** (si configurés, sinon **+** / select pour en ajouter) : raccourcis vers les outils épinglés depuis chaque module (cœur à droite du titre). Puis **Graphiques** (juste sous Favoris) : sur **Jour**, interrupteur en haut à droite **Ligne** (timeline tétées terracotta + siestes bleu, curseur de zoom **6 h → 24 h** mémorisé en local ; à 6 h les dernières heures sont à droite, scroll vers la gauche ; à 24 h toute la journée tient sur la largeur) / **Barres** (deux graphes séparés, une barre par tétée et par sieste) ; le choix Ligne/Barres est mémorisé en local (`abel.dash-day-chart`) ; une séance dont le **début est dans le futur** n’est pas affichée ; **durées du jour** (favori Sommeil, timeline, barres, totaux sous les graphes, modules Allaitement et Sommeil) = mêmes minutes, portion après minuit y compris une sieste commencée la veille (bâton à 0 h) ; camembert couches (pipi / caca / les deux). Sur **7 j / 30 j / Tout** : **heures de sieste et repas** d’abord (cadran 24 h à double anneau : repas terracotta à l’extérieur, siestes bleu à l’intérieur ; plus foncé / plus large = plus fréquent ou plus long à cette heure ; moyenne / jour au centre et sous le graphe) ; interrupteur Cadran / **Agenda** (une ligne par jour, **aujourd’hui en haut**, piste repas + piste siestes, heures 0–24 collées au-dessus) mémorisé en local (`abel.dash-clock-view`) ; sur **30 j** et **Tout**, l’agenda défile vers le bas pour le passé. Puis totaux par jour (**dernier jour à droite**, défilement vers la gauche) : **Repas** et **Sommeil** en bâtons seulement sur **7 j** et **Tout** (retirés sur **30 j**, le cadran / agenda suffisent) ; sur **Tout**, repas / sommeil / couches : **3 jours visibles**, bâtons larges ; **Couches** (7 j / 30 j / Tout) : bâton empilé pipi / caca / les deux (mêmes couleurs que le camembert du jour). Sommeil par jour = minutes réellement couvertes ce jour-là (sieste à cheval coupée à minuit). **Repas** (7 j / Tout) : un bâton empilé tétées / biberons (hauteur = nombre, total au-dessus) ; minutes dans le segment tétées, ml dans le segment biberons s’il y en a. **Poids et taille** : dates sur l’axe X, **6 dernières mesures** visibles (scroll vers la gauche pour l’historique), ligne **en pointillés** s’il manque une mesure (ex. taille). IMC indicatif vert / orange / rouge — pas un avis médical. **Accordéon Notes** puis **Alertes** repas / sommeil / couche. Notes filtrées par période (todos ouvertes toujours visibles ; todos faites selon `doneAt`). Puis **Apports** : biberon, diversification, compléments (une ligne par indicateur avec bouton **+**). Puis **Suivi** : couches, sommeil, tire-lait (stock et tiré sur la période), poids/taille/PC, temp. (même format liste avec **+**). **Entrées de la période** : journal synthétique en bas de page (avatar Google à droite si l’auteur est connu) ; **tap pour modifier** (même éditeur que le journal Bébé).
